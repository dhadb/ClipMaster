...
ClipMaster 不上传、不同步、不分析你的剪贴板内容。所有历史记录、设置和图片缓存都只保存在本机的 `%AppData%/ClipMaster/` 目录中。

- **本地保存**：剪贴板历史仅写入本机数据文件，项目不包含任何云端服务。
- **敏感内容保护**：默认跳过密码、Token、私钥、银行卡号等高风险内容。
- **可控清理**：支持清空未收藏记录、清空全部历史，以及清理不再使用的图片缓存。
- **可暂停记录**：需要复制临时隐私内容时，可以暂停剪贴板监控。
