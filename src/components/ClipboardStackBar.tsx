...
const ClipboardStackBar: React.FC = memo(() => {
  ...
  return (
    <div className="relative z-10 flex items-center gap-2 border-b px-3 py-2 fade-in" style={{ borderColor: 'var(--border-divider)', background: 'color-mix(in srgb, var(--color-success) 6%, var(--bg-surface))' }}>
      ...
      <div className="flex min-w-0 flex-1 items-center gap-1 overflow-x-auto" style={{ scrollbarWidth: 'none' }}>
        {items.slice(0, 4).map((item, index) => item && (
          <button key={item.id} onClick={() => removeFromStack(item.id)} className="inline-flex max-w-[120px] items-center gap-1 rounded-md px-1.5 py-1 text-[10px] interactive-chip" style={{ background: 'var(--bg-elevated)', color: 'var(--text-secondary)' }} title={t('stack.remove')}>
